...
import kotlinx.android.synthetic.main.activity_product_diskon_penjualan.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductDiskonPenjualan : Activity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_diskon_penjualan)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        diskon_penjualan_text_view.setHtml(data.data, HtmlHttpImageGetter(diskon_penjualan_text_view))
                    }
                }
            }, {
                err ->
