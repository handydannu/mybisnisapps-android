...
import kotlinx.android.synthetic.main.activity_op_branding_sosmed.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class OpBrandingSosmed : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_op_branding_sosmed)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        op_branding_sosmed_text_view.setHtml(data.data, HtmlHttpImageGetter(op_branding_sosmed_text_view))
                    }
                }
            }, {
                err ->
