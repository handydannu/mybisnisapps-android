...
import kotlinx.android.synthetic.main.activity_product_event_organizer.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductEventOrganizer : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_event_organizer)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        event_organizer_text_view.setHtml(data.data, HtmlHttpImageGetter(event_organizer_text_view))
                    }
                }
            }, {
                err ->
