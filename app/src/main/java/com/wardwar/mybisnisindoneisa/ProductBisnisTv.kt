...
import kotlinx.android.synthetic.main.activity_product_bisnis_tv.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductBisnisTv : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_bisnis_tv)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        bisnis_tv_text_view.setHtml(data.data, HtmlHttpImageGetter(bisnis_tv_text_view))
                    }
                }
            }, {
                err ->
