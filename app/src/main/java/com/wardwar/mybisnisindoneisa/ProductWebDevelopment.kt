...
import kotlinx.android.synthetic.main.activity_product_web_development.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductWebDevelopment : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_web_development)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        web_develop_text_view.setHtml(data.data, HtmlHttpImageGetter(web_develop_text_view))
                    }
                }
            }, {
                err ->
