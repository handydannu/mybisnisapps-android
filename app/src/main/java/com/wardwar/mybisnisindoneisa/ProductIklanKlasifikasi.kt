...
import kotlinx.android.synthetic.main.activity_product_iklan.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductIklanKlasifikasi : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_iklan)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        iklan_text_view.setHtml(data.data, HtmlHttpImageGetter(iklan_text_view))
                    }
                }
            }, {
                err ->
