...
import kotlinx.android.synthetic.main.activity_product_percetakan.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductPercetakan : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_percetakan)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        percetakan_text_view.setHtml(data.data, HtmlHttpImageGetter(percetakan_text_view))
                    }
                }
            }, {
                err ->
