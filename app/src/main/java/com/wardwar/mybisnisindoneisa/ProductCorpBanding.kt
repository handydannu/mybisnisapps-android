...
import kotlinx.android.synthetic.main.activity_product_corp_banding.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductCorpBanding : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_corp_banding)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        corp_branding_text_view.setHtml(data.data, HtmlHttpImageGetter(corp_branding_text_view))
                    }
                }
            }, {
                err ->
