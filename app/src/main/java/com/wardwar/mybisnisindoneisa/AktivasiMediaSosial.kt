...
import kotlinx.android.synthetic.main.activity_aktivasi_media_sosial.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class AktivasiMediaSosial : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_aktivasi_media_sosial)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        aktivasi_medsos_text_view.setHtml(data.data, HtmlHttpImageGetter(aktivasi_medsos_text_view))
                    }
                }
            }, {
                err ->
