...
import kotlinx.android.synthetic.main.activity_product_permintaan_khusus.*
import kotlinx.android.synthetic.main.component_header.*
import org.sufficientlysecure.htmltextview.HtmlHttpImageGetter

class ProductPermintaanKhusus : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_permintaan_khusus)
            ...
            result.fold({data ->
                when {
                    data.statusCode > 400 -> Toast.makeText(applicationContext, data.message, Toast.LENGTH_SHORT).show()
                        ...
                        permintaan_khusus_text_view.setHtml(data.data, HtmlHttpImageGetter(permintaan_khusus_text_view))
                    }
                }
            }, {
                err ->
